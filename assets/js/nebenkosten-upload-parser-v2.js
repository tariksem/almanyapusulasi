...
function moneyMatches(line){
  const rx=/(?<![\d.,])-?(?:\d{1,3}(?:\.\d{3})+|\d+),\d{2}(?!\d)|(?<![\d.,])-?\d+\.\d{2}(?!\d)/g;
...
function findPeriod(lines){
  const dateRx=/(\d{1,2}[.\-/]\d{1,2}[.\-/](?:\d{2}|\d{4}))/g,candidates=[];
  for(let i=0;i<lines.length;i++){
    const score=scorePeriodLine(lines[i]);if(!score)continue;
    const ctx=[lines[i-1]||'',lines[i],lines[i+1]||''].join(' '),full=ctx.match(dateRx)||[];
...
function findSameLineMoney(lines,patterns){
  for(const p of patterns)for(const l of lines){if(!p.test(l))continue;const ms=moneyMatches(l);if(ms.length)return cleanMoney(ms[ms.length-1]);}
  return '';
}
function findBalance(lines){
  for(const l of lines){if(!/nachzahlung|guthaben|erstattung|saldo|zahlbetrag/i.test(l))continue;const ms=moneyMatches(l);if(ms.length)return {value:cleanMoney(ms[ms.length-1]),credit:/guthaben|erstattung/i.test(l)};}
  return {value:'',credit:false};
}
function findPercentPair(lines,ctxRx){
  for(let i=0;i<lines.length;i++){if(!ctxRx.test(lines[i]))continue;const ctx=[lines[i],lines[i+1]||'',lines[i+2]||''].join(' '),ps=Array.from(ctx.matchAll(/(\d{1,3}(?:[.,]\d+)?)\s*%/g)).map(m=>Number(m[1].replace(',','.'))).filter(Number.isFinite);if(ps.length>=2)return {enabled:true,consumption:String(ps[0]),area:String(ps[1])};}
...
function findCostLines(lines,allText){
  ...
  const hasVatColumn=/vor\s*st\.?\s*(?:eur)?|vorsteuer/i.test(allText),out=[],seen=new Set();
  for(const l of lines){
    if(!known.test(l)||skip.test(l))continue;
    const ms=moneyMatches(l);if(!ms.length)continue;
    const amountIndex=hasVatColumn&&ms.length>=2?ms.length-2:ms.length-1,amount=cleanMoney(ms[amountIndex]);
    const firstPos=l.indexOf(ms[0]);let label=(firstPos>0?l.slice(0,firstPos):l).replace(/[|:;]+/g,' ').replace(/\s+/g,' ').trim();
    label=label.replace(/[-–—]+$/,'').trim().slice(0,60);
    const key=label.toLowerCase()+'|'+amount;if(!label||!amount||seen.has(key))continue;
    seen.add(key);out.push({label,tenantShare:amount});if(out.length>=18)break;
  }
  return out;
}
function parseMoneyNumber(v){let s=String(v||'').trim().replace(/\s/g,'');if(!s)return null;if(s.includes(',')&&s.includes('.'))s=s.replace(/\./g,'').replace(',','.');else if(s.includes(','))s=s.replace(',','.');const n=Number(s);return Number.isFinite(n)?n:null;}
function formatMoneyNumber(n){return Number(n).toFixed(2).replace('.',',');}
function parse(text){
  const lines=String(text||'').split(/\r?\n/).map(x=>x.replace(/\s+/g,' ').trim()).filter(Boolean),period=findPeriod(lines);
  ...
  const advances=findSameLineMoney(lines,[/abzgl\.?\s*(?:ihre\s+)?einzahlungen/i,/(?:ihre\s+)?einzahlungen/i,/vorauszahlungen?\s+gesamt/i,/geleistet.*voraus/i,/abschlags?zahlung/i]),bal=findBalance(lines);
  ...
  return {periodStart:period.start,periodEnd:period.end,totalCost:total,advances,balance:bal.value,balanceCredit:bal.credit,costs:findCostLines(lines,text),heating:findPercentPair(lines,/heiz(?:ung|kosten)/i),warmWater:findPercentPair(lines,/warmwasser|warmes wasser/i)};
}
