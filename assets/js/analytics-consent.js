(function () {
  ...
  function sectionLink(section, className) {
    ...
    const aria = active && active.key === section.key ? ' aria-current="page"' : "";
    return '<a class="' + className + (aria ? " is-active" : "") + '" href="' + section.href + '"' + aria + '>' + section.label + "</a>";
  }
  ...
  function renderHeader() {
    ...
    const primary = PRIMARY_KEYS.map(function (key) {
    ...
    header.innerHTML = '<div class="container header-inner">' +
      '<a class="site-brand" href="/" aria-label="Almanya Pusulası ana sayfa">' +
        '<img class="site-brand-logo" src="/assets/brand/almanya-pusulasi-logo-64.png" alt="">' +
        '<span class="site-brand-copy"><strong>Almanya Pusulası</strong><small>Almanya’da yaşam rehberi</small></span>' +
      '</a>' +
      ...
      '<nav class="site-nav" id="site-navigation" aria-label="Ana menü">' +
        '<div class="primary-nav-links">' + primary.map(function (section) { return sectionLink(section, "site-nav-link"); }).join("") + '</div>' +
        '<details class="nav-more"><summary>Tüm Rehberler <span aria-hidden="true">⌄</span></summary><div class="nav-mega"><div class="nav-mega-head"><strong>Tüm konu başlıkları</strong><span>İhtiyacınıza göre bir bölüm seçin</span></div><div class="nav-mega-grid">' + megaItems + '</div></div></details>' +
      '</nav>' +
    '</div>';
    ...
    document.addEventListener("keydown", function (event) {
      ...
      nav.classList.remove("is-open");
      document.body.classList.remove("menu-open");
      const details = header.querySelector(".nav-more");
  ...
  function renderContextBar() {
    ...
    const currentLabel = pageHeading();
    let crumbs = '<a href="/">Ana sayfa</a><span class="crumb-sep">›</span>';

    if (section) {
      const isHub = currentPath() === section.href.toLowerCase();
      if (isHub) {
        crumbs += '<strong>' + section.label + '</strong>';
      } else {
        crumbs += '<a href="' + section.href + '">' + section.label + '</a><span class="crumb-sep">›</span><strong>' + currentLabel + '</strong>';
      }
    } else {
      crumbs += '<strong>' + currentLabel + '</strong>';
    }
  ...
  function renderFooter() {
    ...
    footer.innerHTML = '<div class="container footer-grid">' +
      '<div class="footer-brand"><a href="/" class="footer-brand-name">Almanya Pusulası</a><p>Almanya’da yaşayan ve Almanya’ya gelmek isteyen Türkler için pratik, bağlantılı ve düzenli güncellenen rehberler.</p></div>' +
      '<div><strong class="footer-title">Popüler Rehberler</strong><a href="/goc-kariyer/">Göç & Kariyer</a><a href="/yerlesim/">Yerleşim</a><a href="/finans/">Finans</a><a href="/schufa/">SCHUFA</a><a href="/kredi/">Ev & Kredi</a></div>' +
      '<div><strong class="footer-title">Diğer Konular</strong><a href="/blue-card/">Blue Card</a><a href="/kindergeld/">Kindergeld</a><a href="/vergi/">Vergi</a><a href="/emeklilik/">Emeklilik</a><a href="/turkiye-seyahati/">Türkiye Seyahati</a></div>' +
      '<div class="footer-legal"><strong class="footer-title">Site</strong><a href="/about/">Hakkımızda</a><a href="/contact/">İletişim</a><a href="/privacy/">Gizlilik</a><a href="/impressum/">Impressum</a></div>' +
    '</div><div class="container footer-bottom"><span>© 2026 Almanya Pusulası</span><span>Bağımsız Türkçe Almanya rehberi</span></div>';
  }
  ...
  function createBanner() {
    ...
    banner.querySelector(".cookie-accept").addEventListener("click", function () { saveChoice(ACCEPTED); loadGoogleAnalytics(); removeBanner(); });
    banner.querySelector(".cookie-reject").addEventListener("click", function () { saveChoice(REJECTED); setConsent(false); deleteAnalyticsCookies(); removeBanner(); });
  }

  function addPrivacySettingsButton() {
    const legal = document.querySelector(".footer-legal");
    if (!legal || legal.querySelector(".privacy-settings-button")) return;
    const button = document.createElement("button");
    button.type = "button";
    button.className = "privacy-settings-button";
    button.textContent = "Çerez tercihleri";
    button.addEventListener("click", function () { window.apResetCookieConsent(); });
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    else if (choice !== REJECTED) createBanner();
  });
})();
